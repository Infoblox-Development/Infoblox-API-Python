...
class Infoblox(object):
    ...
    def get_host_by_alias(self, fqdn, fields=None):
        """ Implements IBA REST API call to retrieve HOST records by fqdn or fqdn regexp filter
        Returns a list of HOST records where their 'alias' field matches the 'fqdn' argument.
        :param fqdn: alias in fqdn or fqdn regexp filter
        :param fields: comma-separated list of field names to retrieve (optional)
        """
        if fields:
            rest_url = 'https://' + self.iba_host + '/wapi/v' + self.iba_wapi_version + '/record:host?alias~=' + fqdn + '&view=' + self.iba_dns_view + '&_return_fields=' + fields
        else:
            rest_url = 'https://' + self.iba_host + '/wapi/v' + self.iba_wapi_version + '/record:host?alias~=' + fqdn + '&view=' + self.iba_dns_view
        hosts = []
        try:
            r = requests.get(url=rest_url, auth=(self.iba_user, self.iba_password), verify=self.iba_verify_ssl)
            r_json = r.json()
    ...
    def get_ptr_by_regexp(self, ptrdname):
        """ Implements IBA REST API call to retrieve PTR records by ptrdname regexp filter
        Returns a list of PTR records where the 'ptrdname' field matches the 'ptrdname' argument
        :param ptrdname: ptrdname in FQDN or fqdn regexp filter
        returns a max of 5000 records.
        """
        rest_url = 'https://' + self.iba_host + '/wapi/v' + self.iba_wapi_version + '/record:ptr?ptrdname~=' + ptrdname
        dns_view = self.iba_dns_view
        max_results = 5000
        req_params = {'view': dns_view, '_max_results': str(max_results)}
        ptr_records = []
        try:
            ...
            if r.status_code == 200:
                if len(r_json) > 0:
                    for ptr_rec in r_json:
                        ptr_records.append(ptr_rec)
                    return ptr_records
                else:
                    raise InfobloxNotFoundException("No PTR records found for the specified regexp: " + ptrdname)
            else:
                if 'text' in r_json:
                    raise InfobloxGeneralException(r_json['text'])
                else:
    ...
    def get_a_by_regexp(self, fqdn):
        """ Implements IBA REST API call to retrieve A records fqdn regexp filter
        Returns a list of A records where the 'name' field matches the 'fqdn' argument
        :param fqdn: name in FQDN or fqdn regexp filter
        returns a max of 5000 records.
        """
        rest_url = 'https://' + self.iba_host + '/wapi/v' + self.iba_wapi_version + '/record:a?name~=' + fqdn
        dns_view = self.iba_dns_view
        max_results = 5000
        req_params = {'view': dns_view, '_max_results': str(max_results)}
        a_records = []
        try:
            ...
            if r.status_code == 200:
                if len(r_json) > 0:
                    for a_record in r_json:
                        a_records.append(a_record)
                    return a_records
                else:
                    raise InfobloxNotFoundException("No A records found for the specified regexp" + fqdn)
            else:
                if 'text' in r_json:
                    raise InfobloxGeneralException(r_json['text'])
                else:
    ...
    def get_cname_by_name(self, fqdn):
        """
        Implements IBA REST API call to retrieve CNAME records by fqdn regexp filter
        Returns a list of CNAME's records where their 'name' field matches the fqdn argument.
        :param fqdn: name in FQDN or fqdn regexp filter
        returns a max of 5000 records.
        """
        rest_url = 'https://' + self.iba_host + '/wapi/v' + self.iba_wapi_version + '/record:cname?name~=' + fqdn + '&view=' + self.iba_dns_view
        ...
        try:
            ...
            if r.status_code == 200:
                if len(r_json) > 0:
                    ...
                    raise InfobloxNotFoundException("No CNAME records found for regexp: " + fqdn)
            else:
                if 'text' in r_json:
                    raise InfobloxGeneralException(r_json['text'])
                else:
    ...
    def get_cname_by_canonical(self, canonical):
        """
        Implements IBA REST API call to retrieve CNAME records by fqdn regexp filter
        Returns a list of CNAME's records where their 'canonical' field matches the canonical argument.
        :param canonical: canonical in FQDN or canonical fqdn regexp filter
        """
        rest_url = 'https://' + self.iba_host + '/wapi/v' + self.iba_wapi_version + '/record:cname?canonical~=' + canonical + '&view=' + self.iba_dns_view
        dns_view = self.iba_dns_view
        max_results = 5000
        ...
        try:
            ...
            if r.status_code == 200:
                if len(r_json) > 0:
                    ...
                    raise InfobloxNotFoundException("No CNAME record found for regex: " + canonical)
            else:
                if 'text' in r_json:
                    raise InfobloxGeneralException(r_json['text'])
                else:
    ...
    def get_ptr_by_ip(self, ipv4):
        """ Implements IBA REST API call to retrieve PTR records by ipv4 regexp filter
        Returns a list of PTR RECORDS where their 'ipv4addr' field matches the ipv4 provided as the argument.
        :param ipv4: ip address in IPv4 or ipv4 regexp filter
        returns a max of 5000 records.
        """
        rest_url = 'https://' + self.iba_host + '/wapi/v' + self.iba_wapi_version + '/record:ptr?ipv4addr~=' + ipv4 + '&view=' + self.iba_dns_view
        dns_view = self.iba_dns_view
        max_results = 5000
        req_params = {'view': dns_view,
                      '_max_results': str(max_results)}
        ...
        try:
            ...
            if r.status_code == 200:
                if len(r_json) > 0:
                    ...
                    raise InfobloxNotFoundException("No PTR records found for the specified ip: " + ipv4)
            else:
                if 'text' in r_json:
                    raise InfobloxGeneralException(r_json['text'])
                else:
    ...
    def get_a_recs_by_ip(self, ipv4):
        """ Implements IBA REST API call to retrieve A records by ipv4 regexp filter
        Returns a list of A RECORDS where their 'ipv4addr' field matches the ipv4 provided as the argument.
        :param ipv4: ip address in IPv4 or ipv4 regexp filter
        """
        rest_url = 'https://' + self.iba_host + '/wapi/v' + self.iba_wapi_version + '/record:a?ipv4addr~=' + ipv4 + '&view=' + self.iba_dns_view
        hosts = []
        try:
            r = requests.get(url=rest_url, auth=(self.iba_user, self.iba_password), verify=self.iba_verify_ssl)
            r_json = r.json()
            if r.status_code == 200:
                if len(r_json) > 0:
                    ...
                    raise InfobloxNotFoundException("No A records found for the specified ip: " + ipv4)
            else:
                if 'text' in r_json:
                    raise InfobloxGeneralException(r_json['text'])
                else:
    ...
    def get_hosts_by_ip(self, ipv4):
        """ Implements IBA REST API call to retrieve HOST records by ipv4 regexp filter
        Returns a list of HOST records where their ipv4 matches the ipv4 provided as the argument
        :param ipv4: ip address in IPv4 or ipv4 regexp filter
        """
        rest_url = 'https://' + self.iba_host + '/wapi/v' + self.iba_wapi_version + '/record:host?ipv4addr~=' + ipv4 + '&view=' + self.iba_dns_view
        hosts = []
        try:
            r = requests.get(url=rest_url, auth=(self.iba_user, self.iba_password), verify=self.iba_verify_ssl)
            r_json = r.json()
            if r.status_code == 200:
                if len(r_json) > 0:
                    ...
                    raise InfobloxNotFoundException("No hosts found for the ip specified in the regexp: " + ipv4)
            else:
                if 'text' in r_json:
                    raise InfobloxGeneralException(r_json['text'])
                else:
    ...
    def get_host_by_mac(self, mac_addr):
        """ Implements IBA REST API call to retrieve HOST records by 'mac' regexp filter
        Returns a list of HOST records where their 'mac' field matches the mac addr provided as the argument
        :param mac_addr: mac addr or mac addr regexp filter
        """
        rest_url = 'https://' + self.iba_host + '/wapi/v' + self.iba_wapi_version + '/record:host?mac~=' + mac_addr + '&view=' + self.iba_dns_view
        hosts = []
        try:
            r = requests.get(url=rest_url, auth=(self.iba_user, self.iba_password), verify=self.iba_verify_ssl)
            r_json = r.json()
            if r.status_code == 200:
                if len(r_json) > 0:
                    ...
                    raise InfobloxNotFoundException("No hosts found for Mac Addr filter: " + mac_addr)
            else:
                if 'text' in r_json:
                    raise InfobloxGeneralException(r_json['text'])
                else:
